...
import { MoreHorizontal, Download, Pencil, FolderInput, Copy, Tag, MessageSquareText, ShieldCheck, History, Trash2, RefreshCw } from '@lucide/vue'
import { h } from 'vue'
...
<template>
  <NPopover trigger="click" placement="bottom-end" :show-arrow="false" :width="200">
    ...
    <div class="grid gap-1 py-1">
      ...
      <button v-if="file.parse_status === 'queued' || file.parse_status === 'failed'" class="flex items-center gap-2 rounded-1 px-3 py-2 text-left text-13px text-ink transition-colors hover:bg-primarySoft border-none bg-transparent cursor-pointer w-full" @click="emit('reparse', file)">
        <NIcon :size="16"><RefreshCw /></NIcon><span>重新解析</span>
      </button>
