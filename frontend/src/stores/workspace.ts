...
export const useWorkspaceStore = defineStore("workspace", () => {
  ...
  const indexedFiles = computed(() =>
    snapshot.value.files.filter((file) => file.parse_status === "indexed"),
  );
  ...
  const activeBreadcrumbs = computed(() =>
    buildFolderBreadcrumbs(folders.value, activeFolderId.value),
  );
  const activeKnowledgeBase = computed(
    () =>
      knowledgeBases.value.find(
        (knowledgeBase) => knowledgeBase.id === activeKnowledgeBaseId.value,
      ) ?? null,
  );
  const activeKnowledgeDocuments = computed(() =>
    activeKnowledgeBaseId.value
      ? (knowledgeDocumentsByKbId.value[activeKnowledgeBaseId.value] ?? [])
      : [],
  );
  ...
  async function replyFileAnnotation(
    annotationId: string,
  ...
  async function loadTeamMessages(teamId: string, token?: string) {
    ...
    if (!accessToken) {
      return {
        items: teamMessagesById.value[teamId] ?? [],
        total: teamMessagesById.value[teamId]?.length ?? 0,
      };
    }
  ...
  async function createTeam(payload: WorkspaceTeamCreateInput) {
    ...
    try {
      ...
      folders.value = addFolderToTree(
        removeFolderFromTree(folders.value, detail.root_folder.id).items,
        detail.root_folder,
      );
    ...
    try {
      ...
      if (activeTeamDetail.value?.id === teamId) {
        activeTeamDetail.value = {
          ...
          invites: [
            invite,
            ...activeTeamDetail.value.invites.filter((item) => item.id !== invite.id),
          ],
        };
      }
      apiState.value = "live";
  ...
  async function updateTeam(
    ...
    teamOperationLoading.value = true;
    try {
      // updateWorkspaceTeam imported statically
      const detail = await updateWorkspaceTeam(accessToken, teamId, payload);
      activeTeamDetail.value = detail;
      apiState.value = "live";
      return detail;
    } catch (error) {
      errorMessage.value = "团队信息更新失败";
      throw error;
    } finally {
      teamOperationLoading.value = false;
    }
  ...
  async function deleteTeam(teamId: string) {
    ...
    teamOperationLoading.value = true;
    try {
      // deleteWorkspaceTeam imported statically
      await deleteWorkspaceTeam(accessToken, teamId);
      await loadWorkspace(accessToken);
    } catch (error) {
      errorMessage.value = "解散团队失败";
      throw error;
    } finally {
      teamOperationLoading.value = false;
    }
  ...
  async function leaveTeam(teamId: string) {
    ...
    errorMessage.value = "";
    try {
      // leaveWorkspaceTeam imported statically
      await leaveWorkspaceTeam(accessToken, teamId);
      await loadWorkspace(accessToken);
    } catch (error) {
      errorMessage.value = "离开团队失败";
      throw error;
    } finally {
      teamOperationLoading.value = false;
    }
  ...
  async function createFolder(payload: WorkspaceFolderCreateInput) {
    ...
    try {
      ...
      folders.value = addFolderToTree(
        removeFolderFromTree(folders.value, created.id).items,
        created,
      );
    ...
    try {
      ...
      if (!findFolderById(folders.value, activeFolderId.value)) {
        activeFolderId.value =
          findFolderById(folders.value, fallbackFolderId)?.id ?? findFirstFolderId(folders.value);
      }
      apiState.value = "live";
    } catch (error) {
  ...
  function normalizePermissionRulePayload(
  ...
  function connectWebSocket(token: string) {
    ...
    const url = `${proto}//${location.host}/api/v2/ws?token=${encodeURIComponent(token)}`;
    try {
