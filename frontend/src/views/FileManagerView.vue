...
const {
  activeFolderId, annotationSaving, apiState, copyingFileId, creatingFolder,
  deletingAnnotationId, deletingFileId, deletingFolderId, deletingPermissionRuleId,
  downloadingFileId, fileAnnotationsById, fileFilters, fileListLoading, files,
  fileVersionsById, folderOptions, folders, folderTreeLoading, markingNotificationId,
  notifications, permissionRules, permissionRuleSaving, permissionRulesLoading,
  restoringVersionId, summary, updatingFileId, updatingFolderId, uploadingFile, versionFileId,
...
<template>
  <component :is="layout" :api-state-label="apiStateLabel" :api-state-type="apiStateType" :nav-items="navItems" :unread-notifications="summary.unread_notifications" :notifications="notifications" :marking-notification-id="markingNotificationId" page-title="文件管理" @mark-notification-read="(id: string) => workspace.markNotificationRead(id)">
    ...
    <FileWorkbench
      ...
      @create-permission-rule="workspace.createPermissionRule"
      ...
      @update-folder="(fid: string, p: any) => workspace.updateFolder(fid, p)"
