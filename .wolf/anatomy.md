...
- `app/api/routes.py` — FastAPI `/api/v1` routes for auth login/register/refresh, current-user profile update, folders, files, RAG, tools, agents, workflows, teams, audit, and snapshot (~2100 tok)
- `app/domain/__init__.py` — Domain schema package marker (~10 tok)
- `app/domain/schemas.py` — Pydantic request/response schemas for the MVP API contracts, including auth login/register/refresh/profile and folder/file payloads (~2400 tok)
...
- `app/services/workspace.py` — In-memory MVP domain service with demo auth, token-kind refresh rotation, profile updates, folder tree CRUD, files, RAG citations, tools, agents, workflows, teams, and audit logs (~5400 tok)
...
- `README.md` — Project documentation (~0 tok)
- `tests/test_openapi_export.py` — Backend OpenAPI export contract test for title, version, server URL, workspace snapshot, folder CRUD paths, and file lifecycle paths (~320 tok)
- `tests/test_workspace_api.py` — Backend API contract tests for auth, refresh-token/profile boundaries, folder CRUD/tree validations, file lifecycle/versioning, files, RAG, agent tools, and workflow MVP (~2800 tok)
...
- `src/client/workspace.ts` — Workspace client adapter and demo data using generated OpenAPI types/SDK for snapshot, folder tree/CRUD, file list/upload/download/delete/update/copy/version restore, and auth headers (~4300 tok)
...
- `src/components/workspace/FileWorkbench.vue` — Naive UI file workbench composing folder tree, search filters, upload panel, lifecycle panel, row actions, parse status chips, and grouped mobile file rows (~2300 tok)
...
- `src/stores/workspace.ts` — Pinia setup store for workspace snapshot, narrative, folder tree CRUD/selection/options, file filters/list/upload/download/delete/update/copy/version state, and demo/live/fallback modes using auth session tokens (~2100 tok)
...
- `src/stores/__tests__/workspace.spec.ts` — Store tests for generated-client-backed folder CRUD/tree actions plus file list/upload/download/delete/update/copy/version actions and local snapshot updates (~1500 tok)
...
- `src/views/WorkspaceView.vue` — Route-level Chinese workbench page composing layouts, summary, folder/file lifecycle workbench, RAG, agent/workflow, team, and audit panels (~1000 tok)
